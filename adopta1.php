...
<head>
    ...
    <style>
        ...
        /* ==== AJUSTE DE ALTURA PARA QUE NO SE MONTEN ==== */
        ...
        /* Tarjeta de agregar nueva mascota */
        .add-card {
            min-height: 380px;
        }

        /* Por si alguna imagen cambia de tamaño y daña la tarjeta */
        .adopta-card img {
            height: 200px;
            object-fit: cover;
        }
...
<body>
    ...
    <main>
        ...
        <div class="adopta-grid">
            ...
            <?php
            $sql = "SELECT * FROM animal ORDER BY Anim_id DESC";
            $result = $conn->query($sql);

            if ($result->num_rows > 0) {
                while ($animal = $result->fetch_assoc()) {

                    $imagen = "../../img/" . $animal['Anim_foto'];

                    echo "
        <div class='adopta-card'>
            <img src='$imagen' alt='{$animal['Anim_nombre']}'>
            <h3>{$animal['Anim_nombre']}</h3>
            <p>Raza: {$animal['Anim_raza']}</p>
            <p>Edad: {$animal['Anim_edad']}</p>
            <button onclick=\"window.location.href='../adopt/formAdop.html?id={$animal['Anim_id']}'\">
                Adóptame
            </button>
        </div>
        ";
                ...
                echo "<p style='text-align:center;'>No hay animales registrados aún 🐾</p>";
            }
            ?>

            <!-- Tarjeta para agregar -->
            <div class="adopta-card add-card" onclick="window.location.href='a-adopta.html'">
