...
@section('panel-title', 'Panel Veterinario')
    ...
    <aside class="dashboard-sidebar">
        ...
        <nav class="sidebar-navigation">
            <div class="nav-item-wrapper">
                <a href="{{ route('vet.dashboard') }}" class="nav-link-item active">
            ...
            <div class="nav-group-section">
                ...
                <a href="{{ route('vet.animals') }}" class="nav-link-item">
                    <span class="nav-item-icon">🐾</span>
                    <span class="nav-item-label">Animales</span>
                </a>
                <a href="{{ route('vet.tasks') }}" class="nav-link-item">
                    <span class="nav-item-icon">📝</span>
                    <span class="nav-item-label">Mis Tareas</span>
                </a>
                <a href="{{ route('vet.progress') }}" class="nav-link-item">
                    <span class="nav-item-icon">📈</span>
    ...
    <main class="dashboard-main-panel">
        <header class="main-panel-header">
            <div class="header-welcome-col">
                <h2 class="header-main-title">Bienvenido, Dr(a). {{ Auth::user()->name }} 🐾</h2>
                <p class="header-subtitle-text">Gestiona la salud y bienestar de nuestros rescatados.</p>
            </div>
        ...
        <section class="stats-cards-grid">
            <div class="stat-card card-teal">
                <div class="stat-card-top">
                    ...
                    <div class="stat-title-group">
                        <p class="stat-card-label">Animales bajo cuidado</p>
                        <h3 class="stat-card-value">{{ $stats['animals_under_care'] ?? 0 }}</h3>
                    </div>
                </div>
            </div>
            ...
            <div class="stat-card card-purple">
                <div class="stat-card-top">
                    ...
                    <div class="stat-title-group">
                        <p class="stat-card-label">Historiales Médicos</p>
                        <h3 class="stat-card-value">{{ $stats['medical_histories'] ?? 0 }}</h3>
                    </div>
                </div>
            </div>
        </section>
        ...
        <!-- CONTENT LISTS -->
        <section class="dashboard-content-layouts" style="grid-template-columns: 1fr 1fr 1fr; margin-top: 30px; gap: 24px;">
            <!-- PENDING TASKS -->
            <div class="layout-feed-column">
                <div class="feed-card">
                    <div class="feed-card-header">
                        <h4 class="feed-card-title">📝 Tareas Urgentes</h4>
                        <a href="{{ route('vet.tasks') }}" class="feed-card-link">Ver todas</a>
                    </div>
                    <div class="feed-list">
                        @if($recentTasks && count($recentTasks) > 0)
                            ...
                            @endforeach
                        @else
                            <p style="text-align: center; color: #94a3b8; padding: 20px 0;">No tienes tareas urgentes.</p>
                        @endif
                    </div>
                </div>
            </div>
            ...
            <div class="layout-feed-column">
                <div class="feed-card">
                    <div class="feed-card-header">
                        <h4 class="feed-card-title">🐾 Bajo Cuidado</h4>
                        <a href="{{ route('vet.animals') }}" class="feed-card-link">Ver todos</a>
                    </div>
                    <div class="feed-list">
                        @if(isset($animalsUnderCare) && count($animalsUnderCare) > 0)
                            @foreach($animalsUnderCare as $animal)
                            <div class="feed-item" style="border-left: 3px solid #14b8a6;">
                                <div class="feed-details">
                                    <p class="feed-item-title">{{ $animal->Anim_nombre }}</p>
                                    <p class="feed-item-sub">Raza: {{ $animal->Anim_raza }}</p>
                                </div>
                                <div class="feed-meta" style="display: flex; flex-direction: column; align-items: flex-end; gap: 5px;">
                                    <span class="status-badge bg-teal-light" style="color: #0f766e;">{{ $animal->Anim_estado }}</span>
                                    <a href="{{ route('vet.history', $animal->Anim_id) }}" style="font-size: 0.75rem; color: #16a34a; font-weight: 600; text-decoration: none;">Ver historial →</a>
                                </div>
                            </div>
                            @endforeach
                        @else
                            <p style="text-align: center; color: #94a3b8; padding: 20px 0;">No hay animales bajo tu cuidado aún.</p>
                        @endif
                    </div>
                </div>
            </div>
