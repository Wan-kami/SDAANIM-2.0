...
<div class="admin-container">
    ...
    <div class="professional-header">
        <div class="header-content-vet">
            <a href="{{ route('admin.dashboard') }}" class="btn-back-vet" title="Volver al Panel">
                <span class="back-icon-vet">←</span>
            </a>
            <div>
    ...
    <div style="margin-bottom: 3rem;">
        <h3 style="font-size: 1.3rem; color: #009688; margin-bottom: 1.5rem;">✓ Veterinarios Activos ({{ $veterinarians->count() }})</h3>
                    ...
                    <tr>
                        <td>
                            <div class="user-info-cell">
                                <div class="user-avatar-mini vet-theme">{{ substr($vet->name, 0, 1) }}</div>
                                <div>
                                    <span class="user-name">{{ $vet->name }}</span>
                                    <span class="user-id">Documento: {{ $vet->Usu_documento }}</span>
                                </div>
                            </div>
                        </td>
                        <td>
                            <div class="contact-info-cell">
                                <span>📧 {{ $vet->email }}</span>
                                <span>📱 {{ $vet->Usu_telefono }}</span>
                            </div>
                        </td>
                        <td>
                            <span class="status-badge success">{{ $vet->status }}</span>
                        </td>
                        <td>
                            <div class="action-buttons">
                                @if($vet->status === 'Activo')
                                <form action="{{ route('admin.users.deactivate', $vet->Usu_documento) }}" method="POST" class="inline-form" onsubmit="return confirm('¿Desactivar este veterinario?');">
                                    @csrf
                                    @method('PUT')
                                ...
                                @else
                                <form action="{{ route('admin.users.activate', $vet->Usu_documento) }}" method="POST" class="inline-form" onsubmit="return confirm('¿Activar este veterinario?');">
                                    @csrf
                                    @method('PUT')
...
<style>
    ...
    .admin-header-actions h2 { font-size: 2rem; color: #009688; margin: 1rem 0 0.5rem; }
