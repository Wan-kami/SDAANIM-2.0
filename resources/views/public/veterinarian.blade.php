...
@section('content')
    ...
    <div class="role-page">
        ...
        <div class="role-hero vet">
            <div>
                <div class="page-hero-badge">🩺 Apóyanos</div>
                <h1>Conviértete en Veterinario</h1>
                <p>Si eres profesional en medicina veterinaria y querés ayudar a nuestros peluditos del refugio, tu
                    experiencia es clave para su bienestar y recuperación.</p>
            ...
            <div class="hero-card">
                <img src="{{ asset('img/Veterinarios.jpeg') }}" alt="Veterinario"
                    style="width: 100%; border-radius: 18px; object-fit: cover; height: 100%;">
            </div>
        </div>
        ...
        <div class="form-modal-card vet">
            ...
            <form action="{{ route('inscriptions.store') }}" method="POST">
                ...
                <div class="role-form vet" style="padding: 0; background: none; box-shadow: none;">
                    ...
                    <input type="text" id="vet_especialidad" name="ins_especialidad" value="{{ old('ins_especialidad') }}"
                        placeholder="Ej: Medicina interna, cirugía, anestesia">

                    <label for="vet_experiencia">Años de experiencia</label>
                    <input type="number" id="vet_experiencia" name="ins_experiencia_anos"
                        value="{{ old('ins_experiencia_anos') }}" min="0">

                    <label for="vet_certificado">Certificado o título</label>
                    <input type="text" id="vet_certificado" name="ins_certificado" value="{{ old('ins_certificado') }}"
                        placeholder="Nombre de la institución o título">
                    ...
                    <select id="vet_tipo_ayuda" name="ins_tipo_ayuda" required>
                        ...
                        <option value="Consultas" {{ old('ins_tipo_ayuda') == 'Consultas' ? 'selected' : '' }}>Consultas
                        </option>
                        <option value="Cirugías" {{ old('ins_tipo_ayuda') == 'Cirugías' ? 'selected' : '' }}>Cirugías</option>
                        <option value="Urgencias" {{ old('ins_tipo_ayuda') == 'Urgencias' ? 'selected' : '' }}>Urgencias
                        </option>
                    ...
                    <textarea id="vet_comentario" name="ins_comentario"
                        placeholder="Describe tu experiencia clínica, voluntariados previos o disponibilidad.">{{ old('ins_comentario') }}</textarea>

                    <div style="margin-bottom: 15px; margin-top: 10px; text-align: left;">
                        <label
                            style="font-size: 0.9em; display: flex; align-items: center; gap: 8px; cursor: pointer; color: #475569;">
                            <input type="checkbox" name="accept_data_policy" required
                                style="width: 16px; height: 16px; margin: 0; flex-shrink: 0;">
                            <span>Acepto la <a href="javascript:void(0)" onclick="abrirModalPolitica()"
                                    style="color: #0ea5e9; text-decoration: underline;">Política de Tratamiento de Datos
                                    Personales</a></span>
                        </label>
                    </div>
