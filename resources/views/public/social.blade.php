...
@section('content')
    ...
    <main class="quienes-somos">
        ...
        <section class="banner-quienes" style="background: linear-gradient(135deg, #1a1a2e 0%, #16213e 50%, #0f3460 100%);">
            <div class="page-hero-badge">📱 Comunidad activa</div>
            <h2>Síguenos & Conéctate</h2>
            <p>Estamos en todas las redes. ¡Únete a nuestra comunidad y apoya a los peluditos!</p>
            <div class="page-hero-actions">
                <a href="{{ route('awareness') }}" class="page-hero-btn">Ver concientización</a>
                <a href="{{ route('adopta') }}" class="page-hero-btn page-hero-btn-secondary">Adoptar ahora</a>
            </div>
        </section>
        ...
        <section class="seccion" style="border-left-color: #e1306c; background: #fff5f8;">
            ...
            <div
                style="display:grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap:18px; margin-top:10px;">
        ...
        <section class="seccion" style="border-left-color: #f59e0b; background: #fffbeb;">
            ...
            <div
                style="display:grid; grid-template-columns: repeat(auto-fit, minmax(200px,1fr)); gap:14px; margin-top:8px;">

                <div
                    style="background:#fff; border-radius:14px; padding:18px; text-align:center; box-shadow:0 3px 12px rgba(245,158,11,0.15); border-top:4px solid #f59e0b;">
                ...
                <div
                    style="background:#fff; border-radius:14px; padding:18px; text-align:center; box-shadow:0 3px 12px rgba(245,158,11,0.15); border-top:4px solid #10b981;">
                    <div style="font-size:2em; margin-bottom:8px;">📅</div>
                ...
                <div
                    style="background:#fff; border-radius:14px; padding:18px; text-align:center; box-shadow:0 3px 12px rgba(0,0,0,0.08); border-top:4px solid #ef4444; opacity:.75;">
                    <div style="font-size:2em; margin-bottom:8px;">🚫</div>
        ...
        <section class="seccion" style="border-left-color: #2d7d46; background: #f0fdf4;">
            ...
            <p style="color:#4b7c5f; margin-bottom:20px; font-size:0.95em;">Estas son las opiniones reales de quienes ya
                vivieron el proceso de adopción con nosotros.</p>

            @if($reviews->count() > 0)
                <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(270px,1fr)); gap:16px;">
                    @foreach($reviews as $rev)
                        <div
                            style="background:#fff; border-radius:16px; padding:20px; box-shadow:0 4px 14px rgba(0,0,0,0.07); border-bottom:4px solid #2d7d46;">
                            <div style="display:flex; align-items:center; gap:10px; margin-bottom:10px;">
                                <div
                                    style="width:40px; height:40px; border-radius:50%; background:linear-gradient(135deg,#2d7d46,#4caf50); display:flex; align-items:center; justify-content:center; color:#fff; font-weight:800; font-size:1.1em;">
                                    {{ strtoupper(substr($rev->user->name ?? 'A', 0, 1)) }}
                                </div>
                                <div>
                                    <div style="font-weight:700; color:#1a3a2a; font-size:0.95em;">
                                        {{ $rev->user->name ?? 'Adoptante' }}</div>
                            ...
                            <div style="margin-bottom:8px;">
                                @for($i = 1; $i <= 5; $i++)
                                    <span
                                        style="color:{{ $i <= $rev->rev_estrellas ? '#f59e0b' : '#d1d5db' }}; font-size:1.1em;">★</span>
                                @endfor
                            </div>
                            @if($rev->rev_comentario)
                                <p style="color:#4b5563; font-size:0.88em; line-height:1.6; margin:0; font-style:italic;">
                                    "{{ $rev->rev_comentario }}"</p>
                            @endif
                            @if($rev->solicitud?->animal)
                                <small style="color:#2d7d46; font-weight:600; display:block; margin-top:8px;">🐾 Adopté a
                                    {{ $rev->solicitud->animal->Anim_nombre }}</small>
                            @endif
                        </div>
                    @endforeach
                </div>
